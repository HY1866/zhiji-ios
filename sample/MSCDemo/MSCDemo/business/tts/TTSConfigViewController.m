...
-(void)needUpdateEngineType{
    ...
    if ([type isEqualToString:[IFlySpeechConstant TYPE_LOCAL_XTTS]]) {
...
-(void)needUpdateVcn{
    ...
    //update TTS Speaker
    int vcnIndex= 0;
    if([instance.engineType isEqualToString: [IFlySpeechConstant TYPE_LOCAL_XTTS]]){
...
- (IBAction)onSampleRateSegValueChanged:(id)sender {
    ...
    if (seg.selectedSegmentIndex == 0 ) {
        ...
        if ( [instance.engineType  isEqual: [IFlySpeechConstant TYPE_LOCAL_XTTS]]){
...
- (IBAction)onEngineTypeSegValueChanged:(id)sender {
    ...
    if (seg.selectedSegmentIndex == 1) {
        instance.engineType = [IFlySpeechConstant TYPE_LOCAL_XTTS];
        instance.sampleRate = [IFlySpeechConstant SAMPLE_RATE_16K];
        instance.vcnName = @"xiaoyan";
        UIAlertView * alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"T_Alter", nil)
                                                         message:NSLocalizedString(@"M_TTS_ResLimitxtts", nil)
                                                        delegate:self
                                               cancelButtonTitle:NSLocalizedString(@"B_Ok", nil)
                                               otherButtonTitles:nil];
        
        NSLog(@"姓名是：%@",instance.vcnName);
        [alert show];
...
- (NSUInteger)numberOfItemsInPickerView:(AKPickerView *)pickerView{
    ...
    if([instance.engineType isEqualToString: [IFlySpeechConstant TYPE_LOCAL_XTTS]]){
        count=instance.xttsIdentiferArray.count;
    }
    else{
